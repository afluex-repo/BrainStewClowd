...
namespace BrainStew.Controllers
{
    public class UserController : UserBaseController
    {
        ...
        public ActionResult TopUp()
        {
            ...
            #endregion
            #region TenureMonthBind
            Common comm = new Common();
            List<SelectListItem> ddlTenureMonth = new List<SelectListItem>();
            DataSet dstenure = objcomm.BindTenureMonth();
            if (dstenure != null && dstenure.Tables.Count > 0 && dstenure.Tables[0].Rows.Count > 0)
            {
                int count = 0;
                foreach (DataRow r in dstenure.Tables[0].Rows)
                {
                    if (count == 0)
                    {
                        ddlTenureMonth.Add(new SelectListItem { Text = "Select", Value = "0" });
                    }
                    ddlTenureMonth.Add(new SelectListItem { Text = r["month"].ToString(), Value = r["Pk_PPPId"].ToString() });
                    count++;
                }
            }
            ViewBag.ddlTenureMonth = ddlTenureMonth;
            #endregion
            return View(model);
        }
        [HttpPost]
        ...
        public ActionResult CheckAmount(string MonthId)
        {
            Account obj = new Account();
            try
             {
                obj.Pk_PPPId = MonthId;
                DataSet ds = obj.BindTenureMonth();
                if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count>0)
                {
                    obj.Result = "Yes";
                    obj.BonusPercentage = ds.Tables[0].Rows[0]["BonusPercentage"].ToString();
                    obj.BVPercentage= ds.Tables[0].Rows[0]["BVPercentage"].ToString();
                }
            }
            catch (Exception ex)
            {
